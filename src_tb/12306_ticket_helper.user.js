...
function injectDom() {
	...
	html.push('<p>您好，12306 订票助手（淘宝专版）是专门为淘宝浏览器定制的一款免费扩展，不需要注册即可使用。但因为作者投入了很大的时间和精力，因此如果您愿意捐助，那么捐助后在此依然可以注册，注册将会将授权信息更改为您的个人信息。非常感谢您的支持！</p>');
	html.push('<p style="color: red;">请注意， 12306订票助手本身完全免费，请不要从任何渠道购买本助手，如已购买，请根据此页面的<a href="http://www.fishlee.net/about/" target="_blank">联系方式</a>向作者举报！</p>');
	...
	html.push('<h4 style="font-size:18px; font-weight:bold; margin: 0px; line-height: 26px; border-bottom: 1px dotted #ccc;">12306 订票助手 (淘宝浏览器专版) <small>ver ' + window.helperVersion + '</small></h4>');
	html.push('<p> 12306 订票助手(淘宝浏览器专版) 是基于『12306订票助手』并特别为淘宝浏览器定制的订票辅助工具。<strong>本软件免费开源，无需付费使用，仅接受捐助。</strong> </p>');
	html.push('<p style="color: red;"> <strong>特别提醒！本助手免费发布且不作为独立的软件出售！</strong>请不要通过任何渠道购买本助手，如已购买，请根据此页面的<a href="http://www.fishlee.net/about/" target="_blank">联系方式</a>向作者举报。</p>');
	html.push('<p style="color:purple;"> 回家是一个单纯而简单的心愿，希望我们不会变得太复杂……</p>');
	html.push('<p>作者非常感谢大家的支持和鼓励。详细捐助名单和捐助方式，请<a href="http://www.fishlee.net/soft/44/donate.html" target="_blank">参见这里</a>。 </p>');
...
if (location.host == "dynamic.12306.cn" || (location.host == "www.12306.cn" && location.protocol == "https:")) {
	if (navigator.userAgent.indexOf("TaoBrowser") == -1) {
		alert("很抱歉，这是淘宝浏览器专版12306订票助手，非淘宝浏览器请使用通用版。点击确定查看更多。");
...
<label><input type='checkbox' id='autoCorrentDate' checked='checked' /> 查询日期早于或等于今天时，自动修改为明天</label>\
	...
	(function () {
		...
		html.push("<tr class='fish_sep'><td class='name'>自动选定席别</td><td><select id='preSelectSeat'></select><select id='preselectseatlevel'></select>(作者无法保证上下铺选择一定有效)</td><td class='name autoordertd'><label style='display:none;'><input type='checkbox' id='autoorder'/>自动提交订单</label></td><td class='autoordertd'><p style='display:none;'><img id='randCode' src='https://dynamic.12306.cn/otsweb/passCodeAction.do?rand=randp' /> <input size='4' type='text' id='randCodeTxt' /> （<span style='font-weight:bold;color:red；'>请务必阅读说明！</span>）</p></td></tr>");
		...
		html.push("<p>1. 自动提交订单使用的是自动预定的列表顺序，取第一个有效的车次自动提交订单！请确认设置正确；</p><p>2. 自动提交的席别和联系人请在上方选择，和预设的是一致的；</p><p>3. 请务必输入验证码！</p><p>4 作者无法保证自动提交是否会因为铁老大的修改失效，因此请务必同时使用<b>其它浏览器</b>手动提交订单！否则可能会造成您不必要的损失！</p>");
		html.push("</td></tr>");
		...
		$("#autoorder").click(function () {
			if (this.checked) alert("警告！选中将会启用自动下单功能，并取代自动预定功能，请输入验证码，当指定的车次中的指定席别可用时，助手将会为您全自动下单。\n\n请确认您设置了正确的车次和席别！\n\n但是，作者无法保证是否会因为铁道部的修改导致失效，请使用此功能的同时务必使用传统的手动下单以保证不会导致您的损失！");
			$("#swAutoBook")[0].disabled = this.checked;
	...
	(function () {
		...
		checkTicketsQueue.push(function (result) {
			if (result == 0 || !document.getElementById("closeFuseSearch").checked) return result;
...
function initDirectSubmitOrder() {
	...
	$("td.autoordertd").show();
	var html = "<div id='fishSubmitFormStatus' class='outerBox' style='position:fixed;right:0px;bottom:-100px;'><div class='box'><div class='title'>自动提交订单中</div>\
...
<li style='color:green;'><strong>最后操作时间</strong>：<span>--</span></li></div>\
	...
	top.window.$("#fishSubmitFormStatus").remove();
	top.window.$("body").append(html);

	var tip = top.window.$("#tipScript li");
	var counter = top.window.$("#countEle");
	var status = top.window.$("#fishSubmitFormStatus");
	...
	function getOrderFormInfo(html) {
		...
		data["orderRequest.reserve_flag"] = "A";	//网上支付
...
function initLogin() {
	...
	$(".enter_right").empty().append("<div class='enter_enw'>" +
		...
		"<li id='updateFound' style='display:none;'><a style='font-weight:bold; color:red;' href='http://www.fishlee.net/Service/Download.ashx/44/65/12306_ticket_helper.crx' target='_blank'>发现新版本！点此更新</a></li>" +
	...
	var info = [];
	info.push("已许可于：" + (utility.regInfo.name || "淘宝浏览器用户"));
	...
	info.push(utility.regInfo.typeDesc || "正式版");
...
function updateScriptContentForChrome() {
	...
	updateScipt.src = 'https://github.com/iccfish/12306_ticket_helper/raw/master/version_taobao.js';
